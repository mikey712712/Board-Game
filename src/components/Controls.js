...
export const Controls = ({ playerOne, playerTwo, setPlayerOne, setPlayerTwo, turn, setTurn }) => {
	...
	useEffect(() => {
		const checkAbilities = () => {
			...
			for (let ability of abilities.playerOne) {
				if (ability.turns < 1) {
					abilityObject[ability.name] = ability.default
				} else {
					abilityObject[ability.name] = ability.value
				}
			}
			setPlayerOne({ ...playerOne, ...abilityObject })
		}
		checkAbilities()
	}, [abilities])
	...
	const endTurn = () => {
		if (turn === 1) {
			...
			setAbilities({ ...abilities, playerOne: [playerOneAbilities.filter((e) => e.turns > 0)] })
		} else if (turn === 2) {
			...
			setAbilities({ ...abilities, playerOne: [playerTwoAbilities.filter((e) => e.turns > 0)] })
		}
	}
	...
	const Blocker = () => {
		return <Box onClick={(e) => e.stopPropagation()} position={"absolute"} left="0" top="0" w="100vw" h="100vh" opacity={"0"}></Box>
	}
			...
			<Flex w="100vw" justify={"space-between"} m="20px 0" boxSizing="border-box" p="10px" bg="rgba(0,0,0,0.2)">
				...
				<Flex w="400px" justify={"space-between"}>
					{(turn === 1 && playerOne.name === "Knight") || (turn === 2 && playerTwo.name === "Knight") ? (
						<>
							<Button h="fit-content" onMouseEnter={previewKnightSlash} onMouseLeave={unPreview} onClick={knightSlash}>
								Slash
							</Button>
							<Button h="fit-content" onMouseEnter={previewKnightPierce} onMouseLeave={unPreview} onClick={knightPierce}>
								Pierce
							</Button>
							<Button h="fit-content" onMouseEnter={previewKnightLight} onMouseLeave={unPreview} onClick={knightLight}>
								Light Armor
							</Button>
							{/* <Button h="fit-content" onMouseEnter={previewKnightLunge} onMouseLeave={unPreview} onClick={knightLunge}>
							Lunge Attack
						</Button> */}
						</>
					) : (turn === 1 && playerOne.name === "Flying Eyeball") || (turn === 2 && playerTwo.name === "Flying Eyeball") ? (
						<Button h="fit-content" onMouseEnter={previewEyeShoot} onMouseLeave={unPreview} onClick={eyeShoot}>
							Shoot Tear
						</Button>
					) : null}
